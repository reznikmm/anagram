...
package body Anagram.Grammars_Recursive_Descent is
   ...
   is
      ...
      Follow : Terminal_Set_Per_NT;
   begin
      if not Anagram.Grammars_Checks.Is_Well_Formed
              (Self, Verbose => True)
      then
         Ok := False;
         return;
      elsif not Anagram.Grammars_Checks.Is_L_Attributed (Self) then
         Ok := False;
         return;
      end if;
